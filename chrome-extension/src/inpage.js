 ...
 */
class TechPayInpageProvider extends TechPayEventEmitter {
    ...
    _sendToContentScript(payload, callback = null) {
        ...
        if (this.debug) console.log('TechPayPwaWallet request', id, payload);
        window.postMessage({
            target: 'TechPayPWAwalletBackground',
            data: payload,
            msgId: id,
        }, location.origin);
    }
    ...
    _receiveFromContentScript(event) {
        ...
        if (msg.target !== 'TechPayPWAwalletInpage') return
        if (!msg.data) return
        let data = msg.data;

        if (this.debug) console.log('TechPayPwaWallet response', msg.msgId, data);
        ...
        if (msg.msgId && typeof this.messageCallbacks[msg.msgId] !== 'undefined') {
            ...
            if (data.error) {
                console.log('TechPayPwaWallet RPC error', data);
                callback(data.error, null); // (error, result)
            } else {
                callback(null, data); // (error, result)
            }
            if (this.debug) console.log('TechPayPwaWallet response callback called', msg.msgId, data);
        }
    ...
    _handleAccountsChanged (accounts, isEthAccounts = false, isInternal = false) {
        if (this.debug) console.log('TechPayPwaWallet accountsChanged', accounts);
        this.emit('accountsChanged', accounts)
    }
}

window.ethereum = new TechPayInpageProvider();
