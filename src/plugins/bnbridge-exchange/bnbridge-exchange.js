...
export class BNBridgeExchange {
    ...
    /**
     * Get techpay token from tokens list.
     *
    ...
    async getETHBalance(_ethReceiveAddress, _token) {
        const token = /** @type {BNBridgeToken} */ _token || (await this.getTechPayToken());
        const pData = {
     ...
     * @param {string} [photonAddress]
     * @param {BNBridgeDirection} [direction]
     * @param {BNBridgeToken} [_token]
     * @return {Promise<null>}
     */
    async swapToken({ ethAddress = '', bnbAddress = '', photonAddress = '', direction = 'EthereumToBinance' }, _token) {
        const token = /** @type {BNBridgeToken} */ _token || (await this.getTechPayToken());
        const pData = {
            direction,
            token_uuid: token ? token.uuid : '',
            ...
            if (data) {
                if (data.success) {
                    ...
                    if (
                        ...
                        pData.photon_address !== pData.photon_address.toLowerCase()
                    ) {
                        // resend request with Photon address in lowercase
                        pData.photon_address = pData.photon_address.toLowerCase();
