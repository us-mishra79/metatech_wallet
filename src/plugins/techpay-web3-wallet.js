...
export class TechPayWeb3Wallet {
    ...
    async getTransactionToSign({ from, to, value, memo = '' }) {
        ...
        return {
            ...
            gas: gasLimit,
            gasLimit: gasLimit,
            chainId: TECHPAY_CHAIN_ID,
            gasPrice,
            nonce,
            data: memo ? Web3.utils.asciiToHex(memo) : '0x',
            _error: error || undefined,
            _fee: !error ? this.WEIToTPC(this.getTransactionFee(gasPrice, gasLimit)) : 0,
        };
    }
    ...
    */
    ...
    getRemainingBalance(_balance, _gasPrice, _gasLimit) {
        ...
        return parseFloat(this.WEIToTPC(balance.sub(fee.mul(this.toBN(2)))));
    }
    ...
    getMaxRemainingBalance(_balance, _gasPrice) {
        ...
        return parseFloat(this.WEIToTPC(balance.sub(fee.mul(this.toBN(1)))));
    }
    ...
    /**
     * @param _address
     * @param {('techpay' | 'ethereum' | 'binance')} _blockchain
     * @return {boolean}
     */
    isValidAddress(_address, _blockchain = 'techpay') {
        if (_blockchain === 'techpay' || _blockchain === 'ethereum') {
