...
export default {
    ...
    mounted() {
        ...
        this.blockchain = this.contactData.blockchain || 'techpay';
    },
    ...
    methods: {
        ...
        async checkAddress(_value) {
            ...
            if (!this.$fWallet.isValidAddress(address, blockchain)) {
                if (blockchain === 'techpay') {
                    this.addressErrorMsg = 'Not valid techpay address';
            ...
            if (
                ...
                (blockchain === 'techpay' && this.getAccountAndIndexByAddress(address).index !== -1)
            ) {
